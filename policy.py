...
class BasePolicy:
    ...
    """
    This is the base class for all the policies.
    It returns the action given the state.
...
class GaussianPolicy(BasePolicy):
    ...
    """
    This class implements the Gaussian policy.
    It adds noise sampled from a Gaussian distribution
    to the action selected by the actor network.
...
class SearchPolicy(BasePolicy):
    ...
    def __init__(self, 
        ...
        """
        rb_vec = replay buffer vector storing the observations that will be used as nodes
        in the graph

        pdist = a matrix of pairwise distances between the nodes in the graph.
        Here pdist[i, j] is the distance between the ith and jth nodes in the graph.

        open_loop = if True, only performs search once at the beginning of the episode

        weighted_path_planning = if Treu, uses edge weights to perform path planning

        no_waypoint_hopping = if True, will not try to go to the goal 
    ...
    def get_closest_waypoint(self, state):
        ...
        search_dist = sum([
            ...
            np.expand_dims(rb_to_goal_dist, 1)
        ])
